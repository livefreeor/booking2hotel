...
this.imgFloat = function () {
    ...
    $(".imgFloat").each(function () {
        $(this).hover(function () {
            $("body").prepend("<div id='imgFloat'></div>");
...
function imgFloatedit() {
    ...
    $(".imgFloat").each(function () {
        ...
        $(this).hover(function () {
            ...
            $("#imgFloat")
                .html("<img src='" + imgBig + "' class='preload'>")
                .css("top", ($(this).pageY) + "px")
                .css("left", ($(this).pageX) + "px")
                .fadeIn(100);
        },
		function () {
		    $("#imgFloat").remove();
...
$(document).ready(function () {
    ...
    var ProductKey = "<input type=\"hidden\" name=\"pid\" id=\"pid\" value=\"" + GetValueQueryString("pid") + "\" />";
    $("#form1").prepend(ProductKey);
    $("#content_rate").html("");

    $(".product_detail").clone(true).prependTo("#product_information");
    //$(".product_detail").prependTo("#product_information");



    $("#tophotel_footer").prependTo("#product_footer1");
    $(".content_tophotel_footer_b").first().prependTo("#product_footer2");
    $("#content_tophotel_footer_b").prependTo("#product_footer3");
    $("#bg_end_footer").prependTo("#product_footer4");


    $(".latest_reviews_more").prependTo("#product_review");
    $(".booknow_top").remove();

    $("#lbltext").remove();


    var IsSingle = $("#hd_isSingle").val();
    ...
    //var temp = $("#lbltext").html();
    //var tempEncode = htmlEncode(temp);

    //$("#lbltext").html(tempEncode).remove();
    //tooltip();
    //imgFloatedit();
    ...
    $("#btnBook").click(function () {
        ...
        location.href = $("#form1").attr('action');
        //$("#form1").submit();
        //$("#form1").submit();
    });
    getRateTable();
...
function getRateTable() {
    ...
    ga.src = 'http://www.supernoom.com/engineBooking2/BookingEngine.php?' + $("#form1").find("input,textarea,select,hidden").not("#__VIEWSTATE,#__EVENTVALIDATION").serialize() + '&mm=' + mmAuthen;
    ...
    //ga.src = 'http://www.modern-garages.com/BookingEngine.php?Hddateci=2013-9-27&dateci=2013-9-27&Hddateco=2013-9-28&dateco=2013-9-28&rateExchange=25&pid=3605';
    //alert(ga.src);
    //ga.src = 'http://www.361thailand.com/BookingEngine.php?'+$("#form1").serialize();
...
function checkBooking() {
    ...
    if ($("#adult").val() != null) {
        ...
        if (selAdult > (roomMaxAdult + sumExtraBed) && (selAdult + selChild) > transferMaxAdult) {
            $.scrollTo("#errorRoom", 800, {
                offset: { left: 0, top: -200 }, onAfter: function () {
...
function openMemberSignupForm() {
    window.open("http://www.booking2hotels.com/member/memberSignUp.aspx?pid=" + GetValueQueryString("pid"), "myWindowName", "width=600, height=500,titlebar=no,toolbar=no,location=no");
}
...
function authenComplete() {
    ...
    $("#form1").append('<input type="hidden" id="mm" name="mm" value="' + readCookie("member") + '"/>');
}
...
function b2hLoginBox() {
    var pid = GetValueQueryString("pid");
    ...
    strForm += "</div>";
    strForm += "<input type=\"hidden\" name=\"pid\" id=\"Hdpid\" value=\"" + pid + "\" />";
    strForm += "</form><br/><br/>";
...
function b2hForgotBox() {
    ...
    strForm += "</div>";
    strForm += "<input type=\"hidden\" name=\"pid\" id=\"pid\" value=\"" + pid + "\"/>";
    strForm += "</form><br/><br/>";
    modalBox(600, strForm);
...
function getAgency()
{
    var agencyid = readCookie("cAgencyID");
    if (agencyid != null) {
        var strHTML = '<input type="hidden" id="hdAgencyID" name="hdAgencyID" value="' + agencyid + '" />'
        $('#FormBooking').append(strHTML);
    }
    else {
        var admin = GetValueQueryString("Staff");
        alert("Session timeout. Please login again.");
        if (admin == "BHT")
        {
            window.location = "Agencylist.aspx";
        }
        else
        {
