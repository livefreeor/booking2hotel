...
{
    public class FrontOptionMeal : Hotels2BaseClass
    {
        ...
        public FrontOptionMeal()
        {

        }

        public FrontOptionMeal(int ProductID, DateTime DateStart, DateTime DateEnd)
        {
        ...
        public List<FrontOptionMeal> GetMealList()
        {
            List<FrontOptionMeal> objResultList = new List<FrontOptionMeal>();
            using (SqlConnection cn = new SqlConnection(this.ConnectionString))
            {
                ...
                strCommand = strCommand + " Convert(money,ISNULL((select top 1 splong.supplement from tbl_product_price_longweekend_extra_net splong where splong.condition_id=poc_ex.condition_id and splong.date_supplement between " + sqlDateStart + " and " + sqlDateEnd + " and splong.status=1),0)) as sub_long_weekend";
                ...
                strCommand = strCommand + " and pocp_exp.status=1 and poc_ex.status=1 and po.status=1 and po.cat_id=58";
                ...
                while (reader.Read())
                {
                    ...
                    objResultList.Add(new FrontOptionMeal
                    {
                        ...
                        OptionDetail = HttpContext.Current.Server.HtmlEncode(reader["detail"].ToString().Replace("\n", "").Replace("\r", "")),
                        OptionImage = "",
                        PolicyDisplay = "",
                        PolicyContent = "",
