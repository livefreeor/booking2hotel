        ...
        { 
            ...
            string bookingBank = "";
                ...
                if(paymentInfo.ManageID==1)
                {
                    ...
                    if (paymentInfo.GatewayID == 16)
                    {
                        IDictionary<string, string> parameters = new Dictionary<string, string>();
                        StringBuilder txtCHECKSUM = new StringBuilder();
                       
                        parameters.Add("MERCHANT2", paymentInfo.MerchantID.Trim());
                        parameters.Add("TERM2", paymentInfo.TerminalID.Trim());
                        parameters.Add("AMOUNT2", InsertZero(paymentInfo.Amount).Trim());

                        parameters.Add("URL2", paymentInfo.ResponseUrl.Trim());
                        parameters.Add("RESPURL", "https://www.hotels2thailand.com/bk2thengine/kbank_update.aspx");
                        parameters.Add("IPCUST2", "54.245.239.44");
                        parameters.Add("DETAIL2", paymentInfo.WebsiteName.Trim());
                        parameters.Add("INVMERCHANT", paymentInfo.PaymentBankID.ToString());
                        parameters.Add("FILLSPACE", "YES");
                        ...
                        {
                            bankForm = bankForm   + "<INPUT type=\"hidden\" id=\"" + item.Key + "\" name=\"" + item.Key + "\" value=\"" + item.Value + "\">";
                            txtCHECKSUM.Append(item.Value);
                        }

                        txtCHECKSUM.Append(paymentInfo.MD5);

                        string MD5string = txtCHECKSUM.ToString().CreateMD5Hash().ToLower();

                        bankForm = bankForm + "<input type=\"hidden\" id=\"checksum\"  name=\"checksum\" value=\"" + MD5string + "\">";

                        bankForm = bankForm + "</form>\n";

                        //HttpContext.Current.Response.Write(txtCHECKSUM.ToString() + "###\r\n");
                        //HttpContext.Current.Response.Write(MD5string);
                    ...
                    if ((byte)reader["cat_id"] == 29)
                    {
                        ...
                        if (objAllotUpdate.UpdateAllotment(BookingID))
                        {
                            ...
                            else
                            {
                                ...
                                if ((int)reader["product_id"] == 3605 || (int)reader["product_id"] == 3692)
                                {
