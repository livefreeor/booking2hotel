<%
FUNCTION function_gen_option_title(intOptionID,intDestinationID,strFileName,strOptionTitle,bolShowOption,dateCheckIn,dateCheckOut,intQty,arrPromotion,arrAllot,intAllotment,intType)
'arrPromotion = promotion_id,option_id,type_id,qty_min,day_min,day_discount_start,day_discount_num,discount,free_option_id,free_option_qty,title,detail,day_advance_num
	...
	SELECT CASE intType
		Case 1 'Without date input
			IF bolShowOption Then
				strRoomFile = Replace(strFileName,".asp","_room_"&intOptionID&".asp")
				strRoomFile = LCase(strRoomFile)
				strRoomType = "<a href=""/"& function_generate_hotel_link(intDestinationID,"",1) & "/" & strRoomFile &""" target=""_blank""><u>"& strOptionTitle &"</u></a>"
			Else
			...
			IF IsArray(arrPromotion) Then
				...
				For intCount=0 To Ubound(arrPromotion,2)
					IF Cstr(intOptionID)=Cstr(arrPromotion(1,intCount)) AND Cint(arrPromotion(4,intCount))<=intNight AND Cint(arrPromotion(3,intCount))>=Cint(intQty) AND (Cint(intDayAdvance)>=Cint(arrPromotion(12,intCount)))Then
						strRoomType = strRoomType & "<br><span title=""header=[Special Promotion !] body=["&Trim(arrPromotion(11,intCount))&"]""><img src=""/images/i_promotion.gif""></span>" 
						Exit For
					End IF
				Next
			End IF
	...
	function_gen_option_title = strRoomType
