<%
Function function_display_rate_health(intProduct,intType)
Dim sqlRate
Dim rsRate
Dim arrRate
Dim intRate
...
sqlRate=sqlRate&" (select top 1 sp.destination_id from tbl_product sp where sp.product_id=po.product_id)as destination_id"
	...
	Case 1
		...
		For intRate=0 to Ubound(arrRate,2)
			IF arrRate(5,intRate) Then
				option_title="<a href=""javascript:popup('/"&function_generate_health_check_up_link(arrRate(7,intRate),"",1)&"/"&replace(arrRate(6,intRate),".asp","_health_"&arrRate(0,intRate))&".asp',600,600)"">"&arrRate(1,intRate)&"</a>"
			Else
...
End IF
function_display_rate_health=strDetail
End Function
%>
